...
//			var timer = null;
			...
			conNav.addEventListener('touchmove',function(event){
				...
				if(translateY > 0){
					//比例 ： 逐渐减小
					// scale = 1 - 左边留白/屏幕宽度
					var scale = 1- translateY/conNav.clientHeight;
					//translateY 逐渐增加 ，增加幅度减小  (抛物线)
					//新的translateY = 临界值 + 左边留白*scale；
					translateY = 0 + translateY * scale ;

				}else if(translateY < conNav.clientHeight - conList.offsetHeight){					
					//右边留白 = translateY - 临界值   （正值）
					var over = Math.abs(translateY) - (conList.offsetHeight - conNav.clientHeight)
					// scale = 1 - 右边留白/屏幕宽度
					var scale = 1 - over/conNav.clientHeight;					
					//新的translateY = 临界值 + 右边留白*scale；
					translateY = conNav.clientHeight - conList.offsetHeight - over*scale ;
				}
...
//				console.log(target)
				...
				if(target > 0){
					target = 0;
					type = 'easeOut'
				}else if(target < conNav.clientHeight - conList.offsetHeight){
//					target = document.documentElement.clientHeight - conList.offsetHeight;
					target = conNav.clientHeight - conList.offsetHeight;
					type = 'easeOut'
				};
...
//				console.log(c)
				...
				//防止重复开启定时器
				clearInterval(conNav.timer);
				conNav.timer = setInterval(function(){				
					t++;	
					if(t > d){
						//清除定时器
						clearInterval(conNav.timer);
